#!/usr/bin/env zsh
# FAC-613: non-vacuity driver for lifecycle refusal diagnostics.
#
...
pool_src=cmd/herd/resource_governor.go
worktree_pkg=./cmd/herd/

pool_run='TestLifecycleRefusalRetains|TestLifecycleRefusalPreserves|TestLifecycleCancellationKeeps|TestLifecycleSuccessIsNever|TestCensusStageSummaryIs|TestRefusalWithoutStages'
pool_expect='TestLifecycleRefusalRetainsCensusStages TestLifecycleRefusalPreservesErrorIdentity TestLifecycleCancellationKeepsItsIdentityWithStages TestLifecycleSuccessIsNeverWrapped TestCensusStageSummaryIsBoundedAndPathFree TestRefusalWithoutStagesIsReturnedUnchanged'

go_timeout=${VERIFY_DIAG_GO_TIMEOUT:-300}
if [[ "$go_timeout" != <-> ]] || (( ${#go_timeout} > 4 )) || (( go_timeout < 60 || go_timeout > 1800 )); then
	print -u2 "warning: ignoring unusable VERIFY_DIAG_GO_TIMEOUT, using 300s"
	go_timeout=300
fi
...
report_parent=${VERIFY_DIAG_REPORT_DIR:-$repo_root/.verify-lifecycle-diagnostics-logs}
...
on_signal() {
	...
	print -u2 "verify-lifecycle-diagnostics: terminated by signal"
	exit 143
}
trap on_exit EXIT
...
typeset -a controls
controls=(
"refusal-keeps-error-identity${sep}${pool_src}${sep}${worktree_pkg}${sep}func (e *lifecycleSweepError) Unwrap() error { return e.err }${sep}func (e *lifecycleSweepError) Unwrap() error { return nil } // MUTANT: the refusal loses its identity${sep}TestLifecycleRefusalPreservesErrorIdentity${sep}errors.Is no longer answers for the cause"
"success-never-becomes-a-refusal${sep}${pool_src}${sep}${worktree_pkg}${sep}	if err == nil {
		return nil
	}
	summary := censusStageSummary(report.Stages)${sep}	if false && err == nil { // MUTANT: a successful sweep is wrapped as a refusal
		return nil
	}
...
"summary-quotes-no-raw-text${sep}${pool_src}${sep}${worktree_pkg}${sep}		if strings.TrimSpace(stage.CursorError) != \"\" {
			b.WriteString(\" cursor_error\")
		}${sep}		if strings.TrimSpace(stage.CursorError) != \"\" {
			b.WriteString(\" \" + stage.CursorError) // MUTANT: the raw cursor text reaches the refusal
		}${sep}TestCensusStageSummaryIsBoundedAndPathFree${sep}summary leaked a path"
)
...
note "verify-lifecycle-diagnostics: every control killed its named oracle on its named assertion"
