...
# ---------------------------------------------------------------------------
mutations=(
"complete-context-must-keep-caller-allowance${sep}pkg/mergeadmit/complete.go${sep}./pkg/mergeadmit/${sep}	ctx, cancel := ensureProofBudget(ctx)${sep}	ctx, cancel := withProofBudget(context.Background(), ProofBudget{}) // MUTANT: discard the caller context${sep}TestCompleteContextPreservesCancellationAndSpentAllowance${sep}CompleteContext escaped the stopped entry context"
# Three INDEPENDENT charge sites, three controls. An end-to-end "the proof
# refuses" killer cannot attribute a charge: remove one site and the other two
