...
"broken-cursor-is-invisible${sep}${governor_src}${sep}	listStage.CursorError = registeredCursorDiagnostic(g.registeredCursorReadErr, g.registeredCursorPersistErr)${sep}	_ = registeredCursorDiagnostic // MUTANT: every cursor diagnostic is hidden${sep}TestGovernorCensusCursorWriteFailureIsVisibleAndChangesNoLane${sep}a failed cursor advance must surface as a partial diagnostic on the stage"
"cursor-read-failure-is-discarded${sep}${governor_src}${sep}		if !errors.Is(err, os.ErrNotExist) {
			g.registeredCursorReadErr = fmt.Sprintf(\"registered census cursor read: %v\", err)
		}${sep}		_ = errors.Is(err, os.ErrNotExist) // MUTANT: read failures are silently discarded again${sep}TestGovernorCensusCursorReadFailureSurvivesASuccessfulAdvance${sep}the lost durable position is invisible"
