...
# ---------------------------------------------------------------------------
suites=(
"./cmd/herd/${sep}^TestHarvestLandedCanonicalDiscoverySharesPublicAllowance\$${sep}TestHarvestLandedCanonicalDiscoverySharesPublicAllowance"
"./cmd/herd/${sep}^(TestHarvestLandedUsesCanonicalLedgerFromLinkedWorktree|TestHarvestCanonicalLedgerSelectsAndPinsTheSameCandidate|TestHarvestCanonicalLedgerDoesNotBorrowDecoyAuthority|TestHarvestCanonicalLedgerHonorsExplicitOverrides|TestHarvestCanonicalDiscoveryRefusesErrorsAndStoppedContexts)\$${sep}TestHarvestLandedUsesCanonicalLedgerFromLinkedWorktree TestHarvestCanonicalLedgerSelectsAndPinsTheSameCandidate TestHarvestCanonicalLedgerDoesNotBorrowDecoyAuthority TestHarvestCanonicalLedgerHonorsExplicitOverrides TestHarvestCanonicalDiscoveryRefusesErrorsAndStoppedContexts"
"./pkg/mergeadmit/${sep}^TestCompleteContextPreservesCancellationAndSpentAllowance\$${sep}TestCompleteContextPreservesCancellationAndSpentAllowance"
...
# ---------------------------------------------------------------------------
mutations=(
"harvest-ledger-discovery-budget-required${sep}cmd/herd/harvest_ledger.go${sep}./cmd/herd/${sep}	root, _, err := gitroot.ProjectRootWithGit(startDir, mergeadmit.BoundedGit(ctx, startDir))${sep}	root, _, err := gitroot.ProjectRootWithGit(startDir, mergeadmit.BoundedGit(context.Background(), startDir)) // MUTANT: discovery escapes the caller allowance${sep}TestHarvestLandedCanonicalDiscoverySharesPublicAllowance${sep}public entry did not charge discovery before carrier selection"
"harvest-canonical-ledger-required${sep}cmd/herd/harvest_ledger.go${sep}./cmd/herd/${sep}	return reviewledger.PathFor(root), nil${sep}	root = \".\" // MUTANT: canonical discovery discarded for the invoking worktree
	return reviewledger.PathFor(root), nil${sep}TestHarvestLandedUsesCanonicalLedgerFromLinkedWorktree${sep}public landed entry lost canonical exact-candidate evidence"
"ordered-match-stops-later-diff-inspection${sep}pkg/mergeadmit/reconcile.go${sep}./cmd/herd/${sep}	landedContent := make([]string, 0, len(landedCommits))${sep}	// MUTANT: restore the eager full-history emptiness scan before matching.
	if _, err := nonEmptyCommits(ctx, repoDir, landedCommits); err != nil {
		return nil, err
	}
	landedContent := make([]string, 0, len(landedCommits))${sep}TestRunHarvestVerifyLandedStopsInspectingAfterOrderedMatch${sep}complete ordered landing spent its allowance inspecting later history"
