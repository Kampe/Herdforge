...
"disposition-must-follow-the-seal${sep}cmd/herd/reviewingest.go${sep}./cmd/herd/${sep}	receipt, err := gate.ReconcileLandedContext(ctx, req)
	...
	}${sep}	if _, e := hsync.WriteLandedDisposition(\".\", hsync.LandedDisposition{Ref: req.Ref, CandidateSHA: candidate, MergeSHA: proof.MergeSHA, Branch: branch, Method: proof.Method}); e != nil { // MUTANT: the disposition is recorded before the seal
		return e
	}
