...
"owning-roots-canonical-before-state-access${sep}${pool_src}${sep}${worktree_pkg}${sep}	abs, err := filepath.Abs(path)${sep}	abs, err := path, error(nil) // MUTANT: the owning roots keep the caller's spelling${sep}TestRelativeConstructorAnchorsBeforeTheCallerMoves${sep}stopped resolving against the owning repository"
