...
type ownedSubprocess struct {
	...
	stopped  chan struct{}
	stopOnce sync.Once
	// finalizeFn is nil in production and therefore resolves to the current
	// ownership finalizer. Fake tests may inject a per-object decision without
	// mutating process-global execution state.
...
func (o *ownedSubprocess) RunProtocol() (userExitHint int, err error) {
	...
	ready := len(fields) == 1 && fields[0] == "start"
	if o.readyFn != nil {
		ready = o.readyFn(fields)
	}
	if !ready {
...
func (o *ownedSubprocess) Close() error {
	...
	if err := o.reapForExecution(true); err != nil && stopErr == nil {
		stopErr = err
	}
	o.mu.Lock()
	handles := make([]ownedHandle, 0, len(o.handles))
...
// finalizeOwnedForExecution is the production finalization boundary used by
// Verifier.Execute. It remains production-backed by default and is injectable
// only for hermetic mutation proofs.
func finalizeOwnedForExecution(o *ownedSubprocess) error { return finalizeOwnedTree(o) }

func (o *ownedSubprocess) finalizeForExecution() error {
	if o.finalizeFn != nil {
		return o.finalizeFn()
	}
	return finalizeOwnedForExecution(o)
}

func (o *ownedSubprocess) reapForExecution(includeLeader bool) error {
	if o.reapFn != nil {
		return o.reapFn(includeLeader)
	}
	return o.killTracked(includeLeader)
}
