...
// Returns whether the family is BACKED BY A LAUNCH RECEIPT reaching this exact
// commit. That answer is the caller's licence to skip the corroboration gate:
// a family resolved from provenance is already proven, and re-checking it
// against the ledger downgrades it to unrecorded on every FIRST review, because
// the ledger record that would corroborate a SHA is created BY ingesting a
// verdict for that SHA. That defect made a real CHA-3211 candidate unmergeable
// despite having a valid receipt.
