...
// CoordinatorOperation identifies a coordinator-loop operation that may
// launch or route a child. Keeping these names at the routing boundary makes
// the operation-to-shape contract auditable instead of relying on a lane's
// incidental default.
type CoordinatorOperation string

const (
	CoordinatorOperationDispatch     CoordinatorOperation = "dispatch"
	CoordinatorOperationReview       CoordinatorOperation = "review"
	CoordinatorOperationVerification CoordinatorOperation = "verification"
	CoordinatorOperationRecovery     CoordinatorOperation = "recovery"
	CoordinatorOperationPulseRead    CoordinatorOperation = "pulse-read"
	CoordinatorOperationRoutingCheck CoordinatorOperation = "routing-check"
)

var coordinatorOperationShapes = map[CoordinatorOperation]string{
...
// CoordinatorOperationShape returns the explicit shape for a coordinator
// child operation. Unknown operations are rejected by returning false; a
// caller must never silently fall back to the coordinator/frontier shape.
func CoordinatorOperationShape(operation CoordinatorOperation) (string, bool) {
	shape, ok := coordinatorOperationShapes[operation]
	return shape, ok
}
