...
func (l *Ledger) PassSHAs() ([]string, error) {
	...
	launch := indexProjectionEvent(rows, string(EventRecord), false)
	latest := indexProjectionEvent(rows, string(EventVerdict), false)
	...
	for sha, vset := range shaVerdicts {
		...
		for _, verdict := range vset {
			...
			if verdict.Verdict == string(VerdictFAIL) || verdict.Verdict == string(VerdictBLOCKED) {
				if supersededReviewers[rowProjection(verdict)] {
					continue
				}
				hasVeto = true
			}
