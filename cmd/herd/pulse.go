...
// Pane acquisition is deliberately independent of raw status: the same body
// also feeds context-warning detection, which applies to non-Busy lanes.
func classifyPulseAndStatusAgent(agent herdr.AgentEntry, readPane paneTailReader) (herdr.AgentEntry, string) {
	if strings.TrimSpace(agent.PaneID) == "" || readPane == nil {
		return agent, ""
	}
