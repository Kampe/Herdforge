...
var subcommandUsage = map[string]string{
	...
	"review-ingest":    "Usage: herd review-ingest (<verdict-artifact>... | --sweep) [--dry-run] [--json] [--reresolve-provenance]\n  Validate, admit, and audit reviewer verdict artifacts.\n  --sweep: ingest every inbox verdict the ledger has never recorded. Without it\n  artifacts must be named explicitly, so a verdict nobody enumerates stays inert.\n  --reresolve-provenance: for an explicitly named historical duplicate, append\n  receipt-backed provenance only when a genuine launch receipt reaches its SHA.",
