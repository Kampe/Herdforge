...
var subcommandUsage = map[string]string{
	...
	"bundle-reclaim":   "Usage: herd bundle-reclaim --root DIR --manifest FILE [--dry-run|--act] [--json] [--lock-dir DIR] [--lock-wait DUR] [--max-files N] [--max-bytes N] [--min-age DUR] [--protect NAME,NAME]\n  Evidence-backed reclamation of owned git transfer bundles. Dry run by\n  default. FILE is the coordinator-owned retention manifest inside the\n  bundle directory; repository location and a .bundle suffix never prove\n  ownership. The pass holds the native shared-checkout lock (--lock-dir,\n  default .git/herd-shared-checkout.lock.d), serializing reclaimers against\n  participating transfers. A bundle is reclaimable only when it lives inside\n  owned .herd state of the canonical repository, is listed in the manifest,\n  is a regular non-symlink file with link count one, passes git bundle\n  verify, every contained tip is retained by canonical refs (strict\n  list-heads parsing; malformed output retains), and lsof proves no reader\n  holds it open. Identity, parent directory, and state are revalidated\n  immediately before a directory-relative unlink; errors, unknown readers,\n  changed files or parents, hard-linked inodes, timed-out or overflowing\n  children, and missing or unique objects retain the bundle with an\n  actionable reason. Directory iteration and child process lifetimes are\n  bounded. Receipt logs, matrices, recovery records, and non-.bundle files\n  are never candidates. A dry-run count is never reported as freed space.",
	"bundle-manifest":  "Usage: herd bundle-manifest --root DIR --bundle NAME [--bundle NAME...] --authority DECLARED [--out FILE] [--write|--dry-run] [--lock-dir DIR] [--lock-wait DUR]\n  Native producer of the version-2 content-bound retention manifest that\n  herd bundle-reclaim consumes. Bundles are explicit exact .bundle filenames\n  only — never enumerated, globbed, or auto-listed. Each named bundle must\n  be a regular non-symlink, non-hard-linked file inside owned .herd state\n  whose git bundle verifies with every tip retained by canonical refs.\n  Identity (streaming sha256 digest, byte size, integer nanosecond\n  modification time) is captured under the native shared-checkout lock;\n  any refusal fails the whole pass instead of producing partial authority.\n  Dry run by default: describes the exact intended manifest document.\n  --write creates it atomically inside the owned root with O_EXCL — an\n  existing artifact is never overwritten and a symlink destination is never\n  followed. No bundle is ever deleted, moved, or modified.",
	...
	"maintenance":      "Usage: herd maintenance [--act] [--interval <dur>] [--max-cycles <n>]\n  Unattended carrier for the FAC-805 cleanup beat, for repositories where the\n  beat's existing callers cannot run: pulse and daemon both gate on fleet\n  admission and the daemon also dispatches lanes. Drives the same bounded tick\n  under the same kernel tick lock, so it defers to a live pulse beat instead of\n  racing it. No admission gate, no task provider, no dispatch. Default is one\n  cycle then exit, for a supervisor that owns the cadence; --interval makes it\n  resident. Observe-only without --act.",
	...
	"review-ledger": "Usage: herd review-ledger list|queued|pending|tier <sha>|readiness <sha>...|drift|evidence-gap|host-ingest [--with-board]\n" +
		...
		"  host-ingest --candidate SHA --reviewer NAME --receipt FILE [--artifact FILE] [--base SHA]: append-only authenticated identity and builder-provenance reconciliation. Host/family come from the canonical accepted launch log, not flags or caller JSON.",
	...
	"review-bind-evidence": "Usage: herd review-bind-evidence <REF> --candidate <sha> --receipt <digest>\n" +
		...
		"  Corpus/sweep mode is refused. Binding is not completion authority.",
