...
var subcommandUsage = map[string]string{
	...
	"bundle-reclaim":   "Usage: herd bundle-reclaim --root DIR [--dry-run|--act] [--json] [--max-files N] [--max-bytes N] [--min-age DUR] [--protect NAME,NAME]\n  Evidence-backed reclamation of owned git transfer bundles. Dry run by\n  default. A bundle is reclaimable only when it lives inside owned .herd\n  state of the canonical repository, is a regular non-symlink file, passes\n  git bundle verify, every contained tip is retained by canonical refs, and\n  lsof proves no reader holds it open. Identity and state are revalidated\n  immediately before unlink; errors, unknown readers, changed files, and\n  missing or unique objects retain the bundle with an actionable reason.\n  Receipt logs, matrices, recovery records, and non-.bundle files are never\n  candidates. A dry-run count is never reported as freed space.",
